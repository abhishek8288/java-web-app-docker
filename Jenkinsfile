      ...
      stage('Run Docker Image In Dev Server'){
        ...
        def dockerRun = ' docker run  -d -p 8080:8080 --name java-web-app abhishek8288/java-web-app'
         ...
         sshagent(['DOCKER_SERVER']) {
          sh 'ssh -o StrictHostKeyChecking=no ubuntu@13.236.182.139 docker stop java-web-app || true'
          sh 'ssh  ubuntu@13.236.182.139 docker rm java-web-app || true'
          sh 'ssh  ubuntu@13.236.182.139 docker rmi -f  $(docker images -q) || true'
          sh "ssh  ubuntu@13.236.182.139 ${dockerRun}"
       }  
    }
}
